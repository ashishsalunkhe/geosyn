    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-black tracking-[0.2em] text-text-muted uppercase">Live Intelligence Wire</h3>
        </div>
      ...
      <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar">
        <AnimatePresence mode="popLayout">
          {documents.length === 0 ? (
            ...
            >
              <Zap size={24} className="mx-auto mb-3 text-text-muted/20" />
              <p className="text-xs font-bold text-text-muted/60 uppercase tracking-wider italic">Awaiting Ground Intel...</p>
            </motion.div>
          ) : (
            documents.slice(0, 15).map((doc, i) => (
              <motion.div
                ...
                className="relative p-4 rounded-xl glass-panel group hover:bg-foreground/5 transition-all cursor-pointer"
              >
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-[10px] font-mono font-bold text-text-muted/80">
                    {new Date(doc.published_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                
                <p className="text-[13px] font-bold leading-snug text-foreground group-hover:text-primary transition-colors line-clamp-2">
                  {doc.title}
                </p>
              </motion.div>
            ))
